...
const CATEGORY_OPTIONS = [
  { value: 'student council president', label: '학생회장' },
  { value: 'vice student council president', label: '부학생회장' },
  { value: 'manager', label: '부장' },
  { value: 'conductor', label: '차장' },
...
export default function UnionAddPage() {
  ...
  const [category, setCategory] = useState<Category>('student council president');
  const [name, setName] = useState('');
  const [mediaFile, setMediaFile] = useState<File | null>(null);

  const addUnionProfile = useUnionStore((state) => state.addUnionProfile);
  ...
  const uploadFile = useCallback(
    async (file: File, folder: Category) => {
      ...
      const { data } = supabase.storage.from('union-profiles').getPublicUrl(filePath);
  ...
  const handleAddProfile = useCallback(
    () => async () => {
      ...
      try {
        ...
        await addUnionProfile(schoolId, departmentId, name, category, mediaUrl);

        alert('학생회가 성공적으로 추가되었습니다.');
        router.replace(`/admin/${schoolId}/department/${departmentId}?tab=union`);
      } catch (error) {
        console.error('학생회 추가 중 오류가 발생 :', error);
        alert('학생회 추가 중 오류가 발생했습니다.');
        return;
      }
    },
    [addUnionProfile, schoolId, departmentId, name, category, mediaFile, router, uploadFile]
  );

  return (
    <>
      <PageHeader title="학생회 추가하기" />
      <section className="relative bg-white w-full p-4 md:p-10 border border-gray-200 rounded-xl shadow-dropdown md:w-[1080px] md:min-h-[753px]">
        <header className="relative flex flex-col justify-start gap-1">
          <ol className="flex justify-start items-center">
            <li>
              <Badge active>학생회 추가</Badge>
            </li>
          </ol>
          <h3 className="text-24 text-gray-900 font-semibold">학생회 추가</h3>
